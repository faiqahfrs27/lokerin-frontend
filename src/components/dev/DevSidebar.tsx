    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "var(--overlay)", backdropFilter: "blur(2px)" }}>
      <div className="card" style={{ width: "100%", maxWidth: 400, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 10, padding: 24 }}>
          ...
          <button className="btn btn-danger" style={{ flex: 1 }} onClick={onConfirm}>Sign out</button>
        </div>
      </div>
    </div>
  );
...
function Header({ onClose }: { onClose: () => void }) {
  return (
    <div className="admin-side__header">
      ...
      <button className="admin-side__close" onClick={onClose} type="button" aria-label="Tutup menu">
    ...
    <>
      <div className="admin-side__footer">
        ...
        <button
          ...
          style={{ width: "100%", marginTop: 10, fontSize: 12, padding: "7px 12px", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6 }}
      ...
      {showModal && (
        <LogoutModal
          onConfirm={() => { setShowModal(false); logout(); }}
